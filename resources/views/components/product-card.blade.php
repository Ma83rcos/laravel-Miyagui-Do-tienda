<div class="relative bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden product-card ...">
    ...
    <div class="h-56 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center border-b overflow-hidden">
        ...
        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" loading="lazy" class="max-h-full max-w-full object-contain p-4 transition-transform duration-300 hover:scale-105">
        @else
    ...
    <div class="p-6 flex flex-col flex-1">
        ...
        <div class="flex items-center space-x-4 mt-auto">
            ...
            <form action="{{ $isFavorite ? route('favorites.destroy', $product->id) : route('favorites.store', $product->id) }}" method="POST">
                ...
                @method('DELETE')
                @endif
                <button type="submit" title="{{ $isFavorite ? 'Eliminar de favoritos' : 'Añadir a favoritos' }}" class="flex items-center justify-center w-10 h-10 rounded-lg border transition hover:bg-gray-100">
